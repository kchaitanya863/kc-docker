...
fn test_qa_reference_positive_cases() {
    let test_cases = vec![
        ...
        (
            "ghcr.io/homebrew/core/rust:1.75",
            "ghcr.io",
            "homebrew/core/rust",
            "1.75",
            None,
        ...
        (
            "registry.internal.corp:8443/team/app:v2.0",
            "registry.internal.corp:8443",
            "team/app",
            "v2.0",
            None,
        ),
        (
            "alpine@sha256:7144f7e13da1d9",
            "registry-1.docker.io",
            "library/alpine",
            "latest",
            Some("sha256:7144f7e13da1d9"),
        ),
    ];

    for (input, reg, repo, tag, digest) in test_cases {
        let r = ImageReference::parse(input)
            .expect(&format!("Failed to parse valid reference: {}", input));
...
fn test_qa_reference_negative_and_breaking_cases() {
    ...
    for input in invalid_inputs {
        assert!(
            ...
            "Expected error for invalid reference: {:?}",
            input
        );
    }
}
