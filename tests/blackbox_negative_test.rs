...
fn test_i_missing_image() {
    ...
    run_boxr_fail(
        &home,
        &["run", "--rm", "nosuch/image:definitely-missing-xyz", "echo", "hi"],
    );
}
...
fn test_i_nonexistent_container_exec() {
    ...
    run_boxr_fail(
        &home,
        &["exec", "bb-nonexistent-container-xyz", "true"],
    );
}
...
fn test_i_compose_cycle_fails() {
    ...
    let out = run_boxr(&home, &["compose", "-f", fixture.to_str().unwrap(), "up", "-d"]);
